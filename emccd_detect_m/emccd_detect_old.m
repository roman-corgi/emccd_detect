function sim_im = emccd_detect_old(fluxmap, frametime, em_gain, full_well_image,...
...
post_gain_frame = zeros(size(image_frame));
indnz = find(image_frame);
for i = 1:length(indnz)
    ie = indnz(i);
    post_gain_frame(ie) = rand_em_gain_old(image_frame(ie), em_gain);
end
% if cr_rate ~= 0
