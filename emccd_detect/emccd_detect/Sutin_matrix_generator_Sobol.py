...
if __name__ == '__main__':
    ...
    d = 3  # number of input parameters
    gmin = 1.01
    gmax = 10
    Mmin = 50
    Mmax = 900
    ...
    xmin = 1
    xmax = 1000
    xar = np.arange(xmin,xmax)
    bounds = np.array([[gmin, gmax], [Mmin, Mmax], [wmin, wmax]])

    sampler = qmc.Sobol(d=d, scramble=True)
    X_unit = sampler.random_base2(m=10)  # 2^10 = 1024 samples
    X = qmc.scale(X_unit, bounds[:, 0], bounds[:, 1])
    X[:,1] = X[:,1].astype(int)
    X[:,2] = X[:,2].astype(int)

    Y = np.array([exact_gain_PDF(x[0],x[1],x[2],xar) for x in X])

    surrogate = RBFInterpolator(X, Y, kernel="thin_plate_spline")

    #y_fast = surrogate(np.array([[x, y, z]]))[0]
    
    # testing
    X_test_unit = sampler.random(200)
    X_test = qmc.scale(X_test_unit, bounds[:, 0], bounds[:, 1])

    Y_true = np.array([exact_gain_PDF(x[0],x[1],x[2],xar) for x in X_test])
    Y_pred = surrogate(X_test)

    rel_err = np.abs(Y_pred - Y_true) / np.maximum(np.abs(Y_true), 1e-12)
    print(np.percentile(rel_err, [50, 90, 99]))
